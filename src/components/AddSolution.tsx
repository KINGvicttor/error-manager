                ...
                <div className="w-[600px] flex flex-col justify-start items-center bg-primary-white rounded-lg">
                    ...
                    <div className="flex flex-col mt-4">
                        ...
                        <input
                            ...
                            value={solutionCtx?.errorCodeInput}   onChange={(e) => solutionCtx?.setErrorCodeInput(e.target.value)} type="text" pattern="\d*" maxLength={6} minLength={6} placeholder="Digite o código do erro" required/>
                    </div>
                    ...
                    <div className="flex flex-col">
                        ...
                        <input
                            className="bg-secondary-white px-3 py-2 w-[300px] border-primary-blue border-2 rounded-lg outline-0"
                            value={solutionCtx?.errorTitleInput}   onChange={(e) => solutionCtx?.setErrorTitleInput(e.target.value)} type="text" placeholder="Digite o título do erro:" required/>
                    </div>
                    <div className="flex flex-col">
                        <label className="text-sm text-primary-blue mt-4 mb-1" htmlFor="">Digite a solução:</label>
                        <textarea 
                            className="bg-secondary-white px-3 py-2 w-[300px] border-primary-blue border-2 rounded-lg outline-0"
                            value={solutionCtx?.solutionContentInput} onChange={(e) => solutionCtx?.setSolutionContentInput(e.target.value)} placeholder="Digite a solução:"></textarea>
                    </div>
                    <button onClick={() => solutionCtx?.addSolution(solutionCtx?.errorCodeInput, solutionCtx.errorTitleInput, solutionCtx.solutionContentInput)} className="bg-green-500 w-[150px] mt-4 mb-4 px-3 py-2 rounded-lg text-white cursor-pointer hover:bg-green-500/90">+ Add</button>
                </div>
            </div>
        </section>
    )
